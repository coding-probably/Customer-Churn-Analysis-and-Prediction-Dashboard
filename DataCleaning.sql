...
-- Importing CSV to Database --> by importing it to table

-- Understanding the Data --
SELECT Gender, Count(Gender) as TotalCount,
Count(Gender) * 100.00 / (Select Count(*) from customer_data)  as Percentage
from customer_data
Group by Gender;

SELECT Contract, Count(Contract) as TotalCount,
Count(Contract) *  100.00 / (Select Count(*) from customer_data)  as Percentage
from customer_data
Group by Contract;
...
Count(State) * 100.00 / (Select Count(*) from customer_data)  as Percentage
from customer_data
Group by State
...
-- Checking the NULL values by counting them --
...
-- Converting Spaces (Text) to Null --
...
-- Creating a new table "prod_Churn", with all null converted to 'None', 'No' --
...
-- Exporting tables to Excel to be used in POWER BI & Python of machine learing